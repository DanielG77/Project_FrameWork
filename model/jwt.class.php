...
class jwt {
    ...
    public static function generate_token_secure($longitud) {
            ...
            $bytes = (int)(($longitud - ($longitud % 2)) / 2);
            return bin2hex(openssl_random_pseudo_bytes($bytes));
        }

    public function generateToken(array $payload, $key, $expiration = 3600) {
        $header = json_encode([
            'alg' => 'HS256',
            'typ' => 'JWT'
        ]);
        ...
        $payload['jti'] = self::generate_token_secure(32); // JWT ID aleatorio
        return $this->encode($header, json_encode($payload), $key);
    }
}
